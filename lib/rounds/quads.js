...
export default {
  ...
  // Keep the live direct/pass pointers valid if the host removes a team mid-round,
  // so the TV/console never point at a team that's gone and the pass can't stall.
  onTeamRemoved(rt, def, teamId, game) {
    delete rt.awarded[teamId];
    if (rt.directTeamId === teamId) {
      rt.directTeamId = rt.status === 'asking' ? directTeamFor(rt.qIndex, def, game) : null;
    }
    const current = rt.passQueue[rt.passPos];
    rt.passQueue = rt.passQueue.filter((id) => id !== teamId && game.teams.has(id));
    const at = rt.passQueue.indexOf(current);
    rt.passPos = at >= 0 ? at : Math.min(rt.passPos, rt.passQueue.length);
    if (rt.status === 'passing' && rt.passQueue.length === 0) {
      rt.status = 'done';
      game.stopTimer();
    }
  },
