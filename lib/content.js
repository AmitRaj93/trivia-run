...
// token -> real image/audio URL/path. Media is served to clients only as opaque
...
// A value worth proxying: a real URL or an absolute /path (not plain text, not
// already proxied). Used for explicit media fields and Match image entries.
function isProxyable(url) {
  return typeof url === 'string' && (/^https?:\/\//i.test(url) || url.startsWith('/')) && !url.startsWith('/img/');
}

function proxify(url) {
  if (!isProxyable(url)) return url; // leave text alone
...
// Replace every media URL/path anywhere in the quiz with its opaque proxy path.
// Covers `image` / `answerImage` / `audio` fields and Match `left`/`right` images.
function proxifyQuiz(node) {
  if (Array.isArray(node)) {
    // Only image-looking entries in arrays (Match columns); leave text entries.
    return node.map((v) => (typeof v === 'string' && isImageUrl(v) ? proxify(v) : proxifyQuiz(v)));
  }
  if (node && typeof node === 'object') {
    const out = {};
    for (const [k, v] of Object.entries(node)) {
      if ((k === 'image' || k === 'answerImage' || k === 'audio') && typeof v === 'string') out[k] = proxify(v);
