...
export function useGameSocket() {
  ...
  const connect = useCallback(() => {
    ...
    ws.onmessage = (ev) => {
      ...
      if (msg.type === S2C.JOINED) {
        ...
        } else if (msg.role === ROLES.HOST && msg.roomCode) {
          // Rejoin the same room on a reconnect rather than minting a new one,
          // so a host network blip doesn't orphan the live game.
          rejoinRef.current = { type: C2S.JOIN, role: ROLES.HOST, roomCode: msg.roomCode, passcode: passcodeRef.current };
        }
  ...
  const create = useCallback(
    (passcode) => {
      passcodeRef.current = passcode; // reused to re-attach as host on reconnect
