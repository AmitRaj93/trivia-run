...
export class Game {
  constructor(roomCode) {
    ...
    // Post-fact answer tracker: every graded result, keyed by round/question/team/slot
    // so re-grading updates in place. Lives on the game (not the per-question `rt`),
    // so it survives navigating between questions and rounds. Host-only in snapshots.
    this.history = new Map();
    this.createdAt = Date.now();
  }

  // Record (or update) one team's graded result for a question. Called by the round
  // engines at every scoring point. `slot` separates a question's main answer from
  // its bonus so both can be tracked. `response` is what the team gave; `answer` is
  // the correct key. Idempotent per key, so re-grading never duplicates a row.
  recordResult({ roundIndex, qIndex, teamId, slot = 'main', question = '', answer = null, response = null, correct = false, points = 0 }) {
    if (roundIndex == null || qIndex == null || !teamId) return;
    const def = this.quiz.rounds[roundIndex] || {};
    const key = `${roundIndex}:${qIndex}:${teamId}:${slot}`;
    this.history.set(key, {
      key,
      roundIndex,
      qIndex,
      slot,
      roundTitle: def.title ?? `Round ${roundIndex + 1}`,
      roundType: def.type ?? '',
      teamId,
  ...
  // Take over an existing team by name from a new device/session — but only if that
  // team's current session is disconnected (so an active rep can't be hijacked).
  // Issues a fresh token, invalidating the old session. Returns {team} | {busy} | {none}.
  takeoverTeam(name) {
    const norm = String(name || '').trim().toLowerCase();
    if (!norm) return { none: true };
    const team = [...this.teams.values()].find((t) => t.name.trim().toLowerCase() === norm);
    if (!team) return { none: true };
    if (team.repConnected) return { busy: true };
    team.repConnected = true;
    team.repToken = randomUUID();
    return { team };
  }
  ...
  static fromData(data) {
    ...
    g.history = new Map((data.history || []).map((r) => [r.key, r]));
  ...
  snapshot(role = 'viewer') {
    ...
    return {
      ...
      // Answer tracker is host-only: it carries every team's responses and the
      // correct keys, which reps/TV must never see mid-game.
      history: role === 'host' ? [...this.history.values()] : [],
      rounds: this.quiz.rounds.map((r, i) => ({ index: i, type: r.type, title: r.title, subtitle: r.subtitle ?? null, quotes: r.quotes ?? null })),
      roundIndex: this.roundIndex,
